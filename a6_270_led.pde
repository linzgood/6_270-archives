...
// added this
int r_previous_score[] = new int[2];
int r_balls_mined[] = new int[2];
...
void setup() {
  ...
  rfmon = new Serial(this, "/dev/ttyUSB2", 19200);
  rfmon.bufferUntil(13);
  
  ledCtrl = new Serial(this, "/dev/ttyUSB3", 38400, 'E', 8, 1);
...
void drawScoreboard() {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    text(str(r_score[i]), x, 107);
       ...
       fill(255);
       textFont(myFont); 
       text(str(r_balls_mined[i]), x, 190);
...
void updateMined(){
  ...
  for(int i = 0; i < 2; i++) {
   ...
   if(r_score[i] - r_previous_score[i] == 40) {
     r_balls_mined[i]++; 
     r_previous_score[i] = r_score[i];
   }
